...
class mbQuery {
	...
	*/
	function mbConnect() {
		$this->_mbSocket = @fsockopen('mm.musicbrainz.org', 80, $errno, $errstr, 10);
		if(!$this->_mbSocket) {
	...
	*/
	function sendQuery($query, $action) {
		...
		fwrite($this->_mbSocket, "Host: mm.musicbrainz.org\r\n");
	...
	*/
	function fileInfoQuery($args) {
		...
		$rawQuery = 	'<?xml version="1.0" encoding="iso-8859-1"?>' .
		...
		$this->sendQuery($rawQuery, "POST /cgi%2dbin/mq%5f2%5f1.pl HTTP/1.0\r\n");
	...
	*/
	function getArtistFromID($artistID) {
		...
		$this->sendQuery('', "GET /mm%2d2.1/artist/$artistID/2\r\n");
		$rawXml = $this->getQueryResponse();
		$this->mbDisconnect();
	...
	*/
	function getAlbumFromID($albumID) {
		...
		$this->sendQuery('', "GET /mm%2d2.1/album/$albumID/4\r\n");
		$rawXml = $this->getQueryResponse();
		$this->mbDisconnect();

		$results = mbrainz_RDF_parse($rawXml);
		if($results === FALSE) {
			$this->__errorMsg = 'Error Parsing RDF';
			return FALSE;
		}
	...
	*/
	function getTrackFromID($trackID) {
		...
		$this->sendQuery('', "GET /mm%2d2.1/track/$trackID/4\r\n");
		$rawXml = $this->getQueryResponse();
		$this->mbDisconnect();
